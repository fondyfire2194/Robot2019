...
public class PathfinderNotifierVision {
    ...
    private static int myLookForVisionSegment;

    public static void startNotifier(int lookForVisionSegment) {
        minTime = 999;
        maxTime = 0;
        timeAverage = 0;
        segmentCounter = 0;
        ...
        thisTime = 0;
        lastTime = 0;
        myLookForVisionSegment = lookForVisionSegment;
    ...
    private static void runTrajectory() {
        ...
        double latchVisionOffset = 0;
        int remainingVisionSegments = 0;
        double visionCompPerSegment = 0;
		boolean visionHasTarget =false;
		double visionXOffset=0;
        if (segmentCounter >= myLookForVisionSegment && visionHasTarget && latchVisionOffset==0){
            remainingVisionSegments = activeTrajectoryLength-segmentCounter;
            latchVisionOffset = visionXOffset;
            visionCompPerSegment = latchVisionOffset/remainingVisionSegments;
        }
        double useYaw =0;
        
        if (visionCompPerSegment == 0 )
             useYaw = -Robot.driveTrain.getGyroYaw();
        else useYaw = -Robot.driveTrain.getGyroYaw() + visionCompPerSegment;
		...
		double angleDifference = Pathfinder.boundHalfDegrees(desired_heading - useYaw);
